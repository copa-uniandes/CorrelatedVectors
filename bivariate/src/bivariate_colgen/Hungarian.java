...
// Tomado de http://shawntoneil.com/index.php/items/code/java@Hungarian@Hungarian.java
...
public class Hungarian {
   ...
   //this by OG
   private double[][] newcosts;

   public Hungarian(double theCosts[][]) {
      costs = theCosts;
      ...
      for (int i = 0; i < costs.length; i++) {
		for (int j = 0; j < theCosts.length; j++) {
   ...
   //This method is by OG
   private double[][] findMin(){
	   newcosts = new double[costs.length][costs.length];
	   ...
	   for (int i = 0; i < costs.length; i++) {
			for (int j = 0; j < costs.length; j++) {
				newcosts[i][j]=costs[i][j]+Math.abs(min);
			}
		}
	   return newcosts;
   }
 ...
 //This method is by OG
   private double[][] intCosts(){
	   newcosts = new double[costs.length][costs.length];
	   for (int i = 0; i < costs.length; i++) {
		for (int j = 0; j < costs.length; j++) {
			newcosts[i][j] = costs[i][j]*10000;
			newcosts[i][j] = Math.round(newcosts[i][j]);
		}
	   }
	   return newcosts;
   }
